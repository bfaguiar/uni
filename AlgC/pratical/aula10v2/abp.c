...
int ABPOddSum (PtABPNode proot){
	PtABPNode Node = proot; PtQueue Queue; int soma = 0;
...
unsigned int ABPMultCount (PtABPNode proot, int pvalue){
	PtABPNode Node = proot; PtStack Stack; int count=0;
...
unsigned int ABPLeavesCount (PtABPNode proot){
	...
	if(proot->PtRight != NULL && proot->PtLeft != NULL){
   		return ABPLeavesCount(proot->PtRight)+ABPLeavesCount(proot->PtLeft);
  	}
  	
  	if(proot->PtRight != NULL){
    	return ABPLeavesCount(proot->PtRight);
  	}

  	if(proot->PtLeft != NULL){
    	return ABPLeavesCount(proot->PtLeft);
  	}
  	Error = OK;
	return 1;
}
...
unsigned int ABPEvenCount (PtABPNode proot){ 
	...
	if (proot->PtLeft != NULL && proot->PtRight != NULL){
		if (proot->Elem % 2 == 0){
			return ABPEvenCount(proot->PtLeft) + ABPEvenCount(proot->PtRight)+1;
		}
		else return ABPEvenCount(proot->PtRight)+ABPEvenCount(proot->PtLeft);
			
	}
	if(proot->PtRight != NULL){
    	if(proot->Elem % 2 == 0){
    		return ABPEvenCount(proot->PtRight)+1;
    	}
    	else return ABPEvenCount(proot->PtRight);
  	}

  	if(proot->PtLeft != NULL){
    	if(proot->Elem % 2 == 0){
    		return ABPEvenCount(proot->PtLeft)+1;
    	}
    	else return ABPEvenCount(proot->PtLeft);
   
 	 }

  	Error = OK;
  	if(proot->Elem % 2 == 0)return 1;
  	else return 0;
}
...
void ABPEvenPrint (PtABPNode proot){ 
  ...
  ABPEvenPrint(proot->PtLeft);
  	if(proot->Elem%2 == 0)printf("%d ",proot->Elem);
  ABPEvenPrint(proot->PtRight);
  Error = OK;
  return ; 
}
