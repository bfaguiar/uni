...
unsigned int ABPEvenCount (PtABPNode proot) {
    ...
    if(proot->PtRight != NULL && proot->PtLeft != NULL)  {
      if(proot->Elem%2==0)
        return ABPEvenCount(proot->PtRight)+ABPEvenCount(proot->PtLeft)+1;
      else
        return ABPEvenCount(proot->PtRight)+ABPEvenCount(proot->PtLeft);
    }
    ...
    if(proot->PtRight != NULL)  {
      if(proot->Elem%2 == 0)
        ...
        return ABPEvenCount(proot->PtRight);
  }
    ...
    if(proot->PtLeft != NULL) {
      ...
      return ABPEvenCount(proot->PtLeft);
    }
