...
class SearchNode:
    def __init__(self,state,parent, cost_from_root=0, depth=0):
        self.state = state
        self.parent = parent
...
class SearchTree:
    ...
    # construtor
    def __init__(self,problem, strategy='breadth',max_depth=None):
    ...
    def search(self):
        while self.open_nodes != []:
            ...
            self.cost += node.cost_from_root
            self.open_nodes[0:1] = []
            lnewnodes = []
            for a in self.problem.domain.actions(node.state):
                newstate = self.problem.domain.result(node.state,a)
                if newstate not in self.get_path(node):
                    newstatecost = node.cost_from_root + self.problem.domain.cost(node, a)
                    newstatedepth = node.depth + 1
                    lnewnodes += [SearchNode(newstate,node,newstatecost,newstatedepth)]
    ...
    def add_to_open(self,lnewnodes):
        if self.strategy == 'breadth':
            ...
            self.open_nodes.extend(lnewnodes)
            self.open_nodes.sort(key=lambda node: node.cost_from_root)
