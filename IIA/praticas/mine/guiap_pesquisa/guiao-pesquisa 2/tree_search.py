...
class SearchDomain(ABC):
    ...
    #def satisfies(self, state, goal): ## cidadades por exemplo.
    #    return state == goal
...
class SearchTree:
    ...
    def get_actions(self, node):
        if node.parent == None:
            return [node.action]
        return [node.action] + self.get_actions(node.parent)
    
    def get_path_professor(self, node):
        if node.parent == None:
            pass
