...
class Cidades(SearchDomain):
    ...
    def actions(self,cidade): # retorna uma lista de todos os caminhos (outros nos) que posso ir de um no. 
        actlist = []
        for (C1,C2,D) in self.connections:
            if (C1==cidade):
               ...
               actlist += [(C2,C1)]
        return actlist 
    def result(self,cidade,action): # retorna o no filho 
        (C1,C2) = action
        if C1==cidade:
            return C2
    def cost(self, state, action): 
        C1, C2 = action
        if C1 == state:
            for cidade1, cidade2, Dist in self.connections:
                if (C1, C2) == (cidade1, cidade2) or (C1, C2) == (cidade2, cidade1):
                    return Dist

    def heuristic(self, state, goal_state):
        return math.hypot(self.coordinates.get(state)[0] - self.coordinates.get(goal_state)[0],self.coordinates[state][1] - self.coordinates[goal_state][1])
...
#t 0
#t = SearchTree(p)
#print(t.ramification)

t = SearchTree(p, 'breadth')
t_search = t.search(100)
print(t_search)
