...
class Cidades(SearchDomain):
    ...
    def cost(self, cidade, action):
        C1, C2 = action
        if C1 != cidade:
            return None # eu estou em cidade logo eu nao posso tomar essa acao se eles forem diferentes. eu nao posso ir de porto para braga estando em aveiro.
        for (c1, c2, D) in self.connections:
            if (C1, C2) == (c1, c2) or (C1, C2) == (c2, c1):
                return D
            return None
    
    def heuristic(self, state, goal_state): # a funcao heuristica vai ajudar a escolher o caminho: posso ver a distancia das coordenadas. dadas duas cidades (state e goal state)) determinar a distancia entre as duas
        c1_x, c1_ y = self.coordinates[state]
